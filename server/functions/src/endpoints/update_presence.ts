...
export const updatePresence = functions.database.ref('/status/{uid}').onUpdate(async (change, context) => {
    ...
    if (status.lastChanged > eventStatus.lastChanged) {
        Logger.log({
            logName: "updatePresence", message: `Presence not updated for ${context.params.uid} because it was overwritten by a newer event. Status: ${JSON.stringify(status)} Event status: ${JSON.stringify(eventStatus)}`,
    ...
    Logger.log({
        logName: "updatePresence", message: `Presence updated for ${context.params.uid}. Status: ${JSON.stringify(status)} Event status: ${JSON.stringify(eventStatus)}`,
        labels: new Map([["userId", context.params.uid]]),
    });
});