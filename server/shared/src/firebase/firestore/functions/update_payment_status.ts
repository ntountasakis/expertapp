...
export async function updatePaymentStatus({ transaction, paymentStatusCancellationReason, centsRequestedAuthorized,
    centsAuthorized, centsRequestedCapture: centsRequestedCapture, centsCaptured: centsCaptured, centsPaid, status, paymentStatusId }:
    {
        ...
        status: PaymentStatusStates, paymentStatusId: string,
    }): Promise<void> {
