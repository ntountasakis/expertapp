...
async function getChatroomPreviewsOneSided({ transaction, uidKey, uid }:
  ...
  for (const docIdx in metadata.docs) {
    ...
    const lastMessage = await transaction.get(getChatroomCollectionRef().doc(chatroomId).collection("chat_messages").orderBy("millisecondsSinceEpochUtc", "desc").limit(1));
